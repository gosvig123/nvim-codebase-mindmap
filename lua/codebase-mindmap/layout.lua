...
function M.compact_layout(graph)
	...
	for node_id, node in pairs(graph.nodes) do
		if node.kind == "Callee" and node.depth == 0 then
			callee_y =
				layout_tree_recursive(graph, node_id, positions, x_root + 50, callee_y, x_offset, y_spacing, false)
		end
	end
...
function M.tree_layout(graph)
	...
	for node_id, node in pairs(graph.nodes) do
		if node.kind == "Callee" and node.depth == 0 then
			callee_y =
				layout_tree_recursive(graph, node_id, positions, x_root + 40, callee_y, x_offset, y_spacing, false)
		end
	end
...
function M.wide_layout(graph)
	...
	for node_id, node in pairs(graph.nodes) do
		if node.kind == "Callee" and node.depth == 0 then
			callee_y =
				layout_tree_recursive(graph, node_id, positions, x_root + 60, callee_y, x_offset, y_spacing, false)
		end
	end
