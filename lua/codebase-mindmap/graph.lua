...
local function is_builtin_or_stdlib(name)
	...
	local framework_names = {
		...
		"create_task",
		"add_api_route",
		"generate_default_error_message",
		"warning",
		"convert",
		"phrase",
	}
...
function M.get_incoming_calls_async(client, item, depth, max_depth, visited, parent_info, callback)
	...
	local timeout = require("codebase-mindmap").config.lsp_timeout or 1000
	...
	client.request("callHierarchy/incomingCalls", { item = item }, function(err, result)
		...
		for _, call in ipairs(result) do
			...
			if depth < max_depth - 1 then
				M.get_incoming_calls_async(client, call.from, depth + 1, max_depth, visited, call_info, function(nested)
...
function M.get_incoming_calls_recursive(client, item, depth, max_depth, visited, parent_info)
	...
	local timeout = require("codebase-mindmap").config.lsp_timeout or 1000
	local result = client.request_sync("callHierarchy/incomingCalls", { item = item }, timeout)
...
function M.get_outgoing_calls_async(client, item, depth, max_depth, visited, parent_info, callback)
	if depth >= max_depth then
		callback({})
		return
	end
	...
	client.request("callHierarchy/outgoingCalls", { item = item }, function(err, result)
		if err or not result then
			callback({})
			return
		end
		...
		for _, call in ipairs(result) do
			...
			if depth < max_depth - 1 then
				M.get_outgoing_calls_async(client, call.to, depth + 1, max_depth, visited, call_info, function(nested)
					for _, nested_call in ipairs(nested) do
						table.insert(calls, nested_call)
					end
					pending = pending - 1
...
function M.get_outgoing_calls_recursive(client, item, depth, max_depth, visited, parent_info)
	...
	local timeout = require("codebase-mindmap").config.lsp_timeout or 1000
	local result = client.request_sync("callHierarchy/outgoingCalls", { item = item }, timeout)
	if not result or not result.result then
		return {}
	end
	...
	for _, client in ipairs(clients) do
		if client.server_capabilities.callHierarchyProvider then
			...
			if prepare_result and prepare_result.result and #prepare_result.result > 0 then
				...
				local max_depth = require("codebase-mindmap").config.max_depth or 3
				incoming_calls = M.get_incoming_calls_recursive(client, item, 0, max_depth, {})
				outgoing_calls = M.get_outgoing_calls_recursive(client, item, 0, max_depth, {})
			end
			break
		end
	end
